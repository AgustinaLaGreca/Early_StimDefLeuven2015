function P = SCHRStim(P, varargin); 
...
[Fcar, OnsetDelay, BurstDur, RiseDur, FallDur, ISI, ...
    FineITD, GateITD, ModITD, SPL, C] ...
    = SameSize(P.Fcar, P.OnsetDelay, P.BurstDur, P.RiseDur, P.FallDur, P.ISI, ...
    P.FineITD, P.GateITD, P.ModITD, P.SPL, P.C);
...
for ichan=1:Nchan,
    ...
    for icond=1:Ncond,
        ...
        w = local_Waveform(P.TypePanel,chanStr, P.Experiment, Fsam, ISI(idx), ...
            ...
            Fcar(idx),P.CF, P.HarLow, P.HarHigh, P.FreqLow, P.FreqHigh, P.C(idx), SPL(idx), ...
            P.AddNoise, ...
...
freq = freq(freq <= HighFreq);
freq = freq(freq >= LowFreq);
if isempty(freq)
      error('Lowest Harmonic (%d) of fundamental frequency (%d Hz) is greater than specified Highest Frequency (%d Hz).',LowHarmonic, Fcar,HighFreq);
      return
end
...
harmonic_numbers = [LowHarmonic:(numel(freq)+LowHarmonic-1)];
StartPhase = StartPhase + ((C*pi*harmonic_numbers.*(harmonic_numbers+1))/(numel(freq)));
